...
> 开发者内部参考文档。对外介绍请见 [README.md](README.md)。
...
```env
OPENAI_MODEL=qwen-flash
LLM_TIMEOUT_CLASSIFY=45
LLM_TIMEOUT_POLISH=180
LLM_CLASSIFY_WORKERS=3
LLM_ENABLE_POLISH=true
```

- 分类慢：换 flash 模型、提高 `LLM_CLASSIFY_WORKERS`
- 润色超时：提高 `LLM_TIMEOUT_POLISH` 或设 `LLM_ENABLE_POLISH=false`
- Bark 无推送：检查 `.env` 是否只有一行 `BARK_KEY`（重复空行会覆盖有效 Key）
